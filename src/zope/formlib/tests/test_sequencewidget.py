...
class SequenceWidgetTestHelper:
...
class SequenceWidgetTest(SequenceWidgetTestHelper, BrowserWidgetTest):
    ...
    def test_hasInput(self):
        self._widget.request.form['field.foo.count'] = '0'
    ...
    def test_list(self):
        ...
        request = TestRequest(form={'field.foo.add': 'Add bar',
                                    'field.foo.count': '0'})
        widget = ListSequenceWidget(
            self.field, self.field.value_type, request)
        ...
        request = TestRequest(form={'field.foo.0.bar': 'Hello world!',
                                    'field.foo.count': '1'})
        widget = ListSequenceWidget(
            self.field, self.field.value_type, request)
        self.assertTrue(widget.hasInput())
        self.assertEqual(widget.getInputValue(), ['Hello world!'])
    ...
    def test_existing(self):
        ...
        widget = TupleSequenceWidget(
            self.field, self.field.value_type, request)
        widget.setRenderedValue(('existing',))
        self.assertFalse(widget.hasInput())
        self.assertRaises(MissingInputError, widget.getInputValue)
        check_list = (
            'checkbox', 'field.foo.remove_0', 'input', 'field.foo.0.bar',
        ...
        self.verifyResult(widget(), check_list, inorder=True)
        widget.setRenderedValue(('existing', 'second'))
        self.assertFalse(widget.hasInput())
        self.assertRaises(MissingInputError, widget.getInputValue)
        check_list = (
            'checkbox', 'field.foo.remove_0', 'input', 'field.foo.0.bar',
    ...
    def test_remove(self):
        request = TestRequest(form={
            'field.foo.remove_0': '1',
            'field.foo.0.bar': 'existing', 'field.foo.1.bar': 'second',
            'field.foo.remove': 'Remove selected items',
            'field.foo.count': '2'})
        widget = TupleSequenceWidget(
            self.field, self.field.value_type, request)
        widget.setRenderedValue(('existing', 'second'))
        self.assertEqual(widget.getInputValue(), ('second',))
        check_list = (
            'checkbox', 'field.foo.remove_0', 'input', 'field.foo.0.bar',
            'existing',
            'checkbox', 'field.foo.remove_1', 'input', 'field.foo.1.bar',
    ...
    def test_anonymousfield(self):
        self.field = Tuple(__name__='foo', value_type=TextLine())
        request = TestRequest()
        widget = TupleSequenceWidget(
            self.field, self.field.value_type, request)
        widget.setRenderedValue(('existing',))
    ...
    def test_usererror(self):
        self.field = Tuple(__name__='foo',
                           value_type=TextLine(__name__='bar'))
        request = TestRequest(form={
            'field.foo.0.bar': '', 'field.foo.1.bar': 'nonempty',
            'field.foo.count': '2'})
        widget = TupleSequenceWidget(
            self.field, self.field.value_type, request)
        ...
        self.assertEqual(data, [None, 'nonempty'])
...
class SequenceDisplayWidgetTest(
    ...
    def test_honors_subwidget(self):
        ...
        self.content.foo = ('first value', 'second value')
        check_list = ['<ol', 'class=', 'testwidget',
                      '<li', 'FIRST VALUE', '</li',
                      '<li', 'SECOND VALUE', '</li',
                      '</ol']
...
def test_suite():
    return unittest.TestSuite((
        unittest.defaultTestLoader.loadTestsFromTestCase(SequenceWidgetTest),
        doctest.DocTestSuite(setUp=setUp, tearDown=tearDown,
                             optionflags=doctest.ELLIPSIS
                             | doctest.NORMALIZE_WHITESPACE
                             | doctest.REPORT_NDIFF),
        unittest.defaultTestLoader.loadTestsFromTestCase(
            SequenceDisplayWidgetTest),
    ))
